    ...
    <UContainer class="relative py-24">
      <div class="grid items-start gap-14 lg:grid-cols-[minmax(0,0.62fr)_minmax(0,1.38fr)]">
        ...
        >
          ...
          <div class="space-y-4">
            <h2 class="text-3xl font-semibold leading-tight text-zinc-900 transition-colors duration-300 md:text-4xl dark:text-white">
              {{ t('blog.title') }}
            </h2>
          ...
          <div class="flex items-start gap-3 rounded-2xl border border-black/10 bg-white/60 p-4 text-sm text-zinc-600 shadow-sm backdrop-blur transition-colors duration-300 dark:border-white/10 dark:bg-white/5 dark:text-white/70">
            <UIcon name="i-heroicons-language-20-solid" class="mt-1 h-5 w-5 shrink-0 text-cyber-green" />
            <p>{{ t('blog.note') }}</p>
          </div>

          <div v-if="showFeedLink || showBlogLink" class="flex flex-wrap gap-3 pt-2">
            <UButton
              v-if="showBlogLink"
              :href="blogUrl"
              target="_blank"
        ...
        <div
          ...
          v-motion
          :initial="{ opacity: 0, y: 40 }"
          :visibleOnce="{ opacity: 1, y: 0, transition: { delay: 0.12, duration: 0.85, ease: 'easeOut' } }"
          :viewport="{ amount: 0.4 }"
        >
          <FeedCards />
        </div>
